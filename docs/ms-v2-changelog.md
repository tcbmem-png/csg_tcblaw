...
### §1.9 Chancellor decision surface + live recalc cascade — **net-new**

- **Types / data model** (`chancellor-decisions.ts`, net-new): `MSChancellorDecision = { factorLetter, decision, customAmount, decidedAt }` with seven-state enum (`none | adopt_obligor | adopt_obligee | split | custom | decline | accept_agreed`). Audit trail captures the *decision* — contribution is derived. Added `chancellorDecisions: Record<MSFactorLetter, ...>` to `MSInputs` (passes through `share` encode/decode automatically).
- **Pure logic** (`chancellor-decisions.ts`): `decisionContribution(row, dec)`, `availableDecisions(inPlay)` (collapses `agree` rows to two buttons, hides `adopt_other` on single-side assertions), `computeChancellorTotals(rows, decisions)` (per-factor + running total + pending tracking), `recordDecision()` (centralized `decidedAt` stamping, clears on revert to "none"), `clampCustomAmount` (±$50,000 bound per spec).
- **UI** (`chancellor-decision-row.tsx`, net-new): filled-pill buttons with selected-state deeper fill, signed contribution previews inline (`Adopt Obligor (+$400)`, `Split Difference (+$200)`), signed custom-amount input revealed only on `custom`, memoized via `React.memo` so only the affected row re-renders on per-decision change.
- **Cascade** (`deviation-reconciliation.tsx`): refactored to render decision row beneath each active factor; resolved rows (`decline` / `accept_agreed`) get `opacity-60` + "ruled: no deviation" caption per the visual spec; chancellor running-total row + cumulative-through-emancipation panel with a 300ms color-flash transition on cumulative change.
- **Tests** (`__tests__/chancellor-decisions.test.ts`, net-new — 13 tests): per-decision unit coverage (adopt/split/custom/accept_agreed/decline + clamping) plus the Berger-equivalent end-to-end fixture spanning all ten factors with mixed in-play states, asserting `totalMonthly` and `cumulativeNetDifference` match the canonical per-factor sum exactly. Suite now 169/169.
- **Scope discipline**: no save-as-final-ruling affordance, no PDF integration, no decision history beyond the in-state audit trail, no multi-party voting — all deferred to Phase 4+ per spec. The chancellor's most recent decision set IS the current ruling.
